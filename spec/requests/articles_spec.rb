    ...
    context "with at least one saved article" do
      before do
        @article = create(:article, source: @articles[0][:source][:name], author: @articles[0][:author],
                                    title: @articles[0][:title], description: @articles[0][:description], url: @articles[0][:url], url_to_image: @articles[0][:url_to_image], publish_at: @articles[0][:publish_at], content: @articles[0][:content], user: User.last)
      end
